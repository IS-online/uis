...
class BookController extends CollegeBaseController
{
    ...
    public function store(AddValidation $request)
    {
        ...
        $request->session()->flash($this->message_success, $this->panel. ' Uspješno dodavanje.');
    ...
    public function inActive(request $request, $id)
    {
        ...
        $request->session()->flash($this->message_success, $row->faculty.' '.$this->panel.' Neaktivan uspješno.');
        return redirect()->route($this->base_route);
    }
    ...
    public function bookStatus(request $request, $id, $status)
    {
        ...
        $request->session()->flash($this->message_success, $row->faculty.' '.$this->panel.' Promjena statusa uspješno.');
        return back();
    }
    ...
    public function addCopies(Request $request)
    {
        ...
        if($start == 0 or $end == 0){
            $request->session()->flash($this->message_warning, $this->panel. ' Pažnja!, Unesite početnu vrijednost veću od 0');
            return back();
        }

        if($start > $end){
            $request->session()->flash($this->message_warning, $this->panel. ' Pažnja!, Unos krajnje vrijednosti je manji od početne. Ispravi to.');
            return back();
        }
        ...
        if ($quantity > 0) {
            ...
            while ($i <= $quantity){
                ...
                if($row){
                    $request->session()->flash($this->message_warning, $this->panel. ' Kôd već postoji Ispravite i pokušajte ponovo.');
                    return back();
                }else{
                    Book::create([
        ...
        $request->session()->flash($this->message_success, $this->panel. ' Kopije su uspješno dodane.');
        return back();
    ...
    public function bulkCopiesDelete(Request $request)
    {
        if ($request->has('bulk_action') && in_array($request->get('bulk_action'), ['delete'])) {
            if ($request->has('chkIds')) {
                ...
                $request->session()->flash($this->message_warning, 'Molimo, označite barem jednu knjigu.');
                return redirect()->route($this->base_route);
            }
    ...
    public function handleImportBook(Request $request)
    {
        ...
        $request->session()->flash($this->message_success,'Knjige su uspješno uvezene');
        return redirect()->route($this->base_route);
