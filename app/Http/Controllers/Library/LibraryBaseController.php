...
class LibraryBaseController extends CollegeBaseController
{
    ...
    public function issueBook(Request $request)
    {
        ...
        if($request->has('book_id') && $request->get('book_id') !== null ){
            ...
            foreach ($unique_book_request as $key => $book){
                ...
                if ($book_eligible <= 0){
                    $request->session()->flash($this->message_warning,'Član ne ispunjava uslove za knjigu koju je preuzeo u ovom trenutku.
                     Već je preuzet maksimalan broj knjiga');
                    return back();
                }
                ...
                if($avability_status = 1){
                    if($book > 0){
                        ...
                        $request->session()->flash($this->message_success,' Knjiga je uspješno izdana.');
                    }else{
                        $request->session()->flash($this->message_warning, 'Knjiga nije izdata. Molimo odaberite kopiju dostupnih knjiga kada knjigu želite izdati.');
                    }

                    $request->session()->flash($this->message_success,' Knjiga je uspješno izdana.');
                }else{
                    $request->session()->flash($this->message_warning, 'Knjiga nije dostupna za izdavanje ili je oštećena ili je izgubljena. Molimo provjerite detalje knjige za više informacija.');
                }
            }
        }else{
            $request->session()->flash($this->message_warning,'Knjiga nije izdana Molimo odaberite dostupne knjige za izdavanje.');
        }
        return back();
    }
    ...
    public function bookRequestCancel(Request $request, $bookId, $memberId)
    {
        ...
        if($bookRequested){
            ...
            $request->session()->flash($this->message_danger, 'Uspješno ste otkazali zahtjev za knjigom.');
        }else{
            $request->session()->flash($this->message_warning, 'Nevažeći zahtjev.');
        }
        return back();
    }
    ...
    public function bookRequestIssue(Request $request, $bookId, $memberId)
    {
        ...
        if($request->has('book_id') && $request->get('book_id') !== null ){
            ...
            foreach ($unique_book_request as $book){
                ...
                $book_eligible = $issue_limit_books - $current_issued;
                if ($book_eligible <= 0){
                    $request->session()->flash($this->message_warning,'Član ne ispunjava uslove za knjigu koju je preuzeo u ovom trenutku. Već je preuzet maksimalan broj knjiga');
                    return back();
                }
    ...
    public function returnBook(Request $request, $id)
    {
        ...
        $request->session()->flash($this->message_success,'Povrat knjige uspješan.');
        return back();
    }
    ...
    public function bookDetail(Request $request)
    {
        ...
        if($availableCopies){
            ...
            $response['error'] = 'Kopije knjiga nisu dostupne za izdavanje. Za više informacija pogledajte detalje knjige.';
        }
