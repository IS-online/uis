    ...
    public function index(Request $request)
    {
        ...
        if($request->all()) {
            $data['student'] = Student::select('students.id', 'students.reg_no', 'students.reg_date',
                ...
                'students.first_name', 'students.middle_name', 'students.last_name', 'students.status', 'students.mjesto_rodjenja', 
                'students.drzava_rodjenja', 'students.opstina_rodjenja', 'students.broj_dosijea', 'students.nacionalnost', 
                'students.status_studenta', 'promjena_prezimena_imena', 'students.dolazak_sa_druge_vsu','students.nostrifikacija',
                'students.datum_ispisa', 'students.promjene_studija', 'students.pohvale_nagrade', 'students.mjere', 'students.datum_zavrsetka_studija')
                ->where(function ($query) use ($request) {
                    ...
                    if ($request->has('fast_finder') && $request->get('fast_finder') !=null) {
                        $query->where('students.reg_no', 'like', '%' . $request->fast_finder . '%')
                                ...
                                ->orWhere('students.mjesto_rodjenja', 'like' , '%'.$request->fast_finder.'%')
                                ->orWhere('students.broj_dosijea', 'like', '%' . $request->fast_finder . '%');
                    }
                })
            ...
            $data['student'] = Student::select('students.id', 'students.reg_no', 'students.faculty', 'students.semester',
                'students.academic_status', 'students.first_name', 'students.middle_name', 'students.last_name', 'students.status', 'students.mjesto_rodjenja',
                'students.drzava_rodjenja', 'students.opstina_rodjenja', 'students.broj_dosijea', 'students.nacionalnost', 'students.status_studenta')
                ->Active()
   ...
   /* public function getStudentsList(Request $request)
    ...
    public function view($id)
    {
        ...
        $data['student'] = Student::select('students.id', 'students.reg_no', 'students.reg_date', 'students.university_reg',
            ...
            'students.mother_tongue', 'students.email', 'students.extra_info', 'students.status', 'students.mjesto_rodjenja', 'students.drzava_rodjenja',
            'students.opstina_rodjenja', 'students.broj_dosijea', 'students.nacionalnost', 'students.status_studenta', 'students.promjena_prezimena_imena',
            'students.dolazak_sa_druge_vsu', 'students.nostrifikacija', 'students.datum_ispisa', 'students.promjene_studija', 'students.pohvale_nagrade',
            'students.mjere', 'students.datum_zavrsetka_studija',
        ...
        if (!$data['student']){
            request()->session()->flash($this->message_warning, "Nije važeći student");
            return redirect()->route($this->base_route);
        }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $data['row'] = Student::select('students.id','students.reg_no', 'students.reg_date', 'students.university_reg',
            ...
            'students.mjesto_rodjenja', 'students.opstina_rodjenja', 'students.nacionalnost', 'students.status_studenta', 'promjena_prezimena_imena',
            'students.drzava_rodjenja', 'students.broj_dosijea', 'students.dolazak_sa_druge_vsu', 'students.nostrifikacija', 'students.datum_ispisa',
            'students.promjene_studija', 'students.pohvale_nagrade', 'students.mjere', 'students.datum_zavrsetka_studija',
            'pd.grandfather_first_name', 'pd.grandfather_middle_name', 'pd.grandfather_last_name', 
            'pd.father_first_name', 'pd.father_middle_name',
            'pd.father_last_name', 'pd.father_eligibility', 'pd.father_occupation', 'pd.father_office', 'pd.father_office_number',
            'pd.father_residence_number', 'pd.father_mobile_1', 'pd.father_mobile_2', 'pd.father_email', 'pd.mother_first_name',
            'pd.mother_middle_name', 'pd.mother_last_name', 'pd.mother_eligibility', 'pd.mother_occupation', 'pd.mother_office',
            'pd.mother_office_number', 'pd.mother_residence_number', 'pd.mother_mobile_1', 'pd.mother_mobile_2', 'pd.mother_email',
    ...
    public function update(EditValidation $request, $id)
    {
        ...
        /*Academic Info End*/
        $request->session()->flash($this->message_success, $this->panel. ' Info izmjenjen uspješno.');
        //return redirect()->route($this->base_route);
        return back();
    ...
    public function delete(Request $request, $id)
    {
        ...
        if($errCount > 0){
            $request->session()->flash($this->message_warning, $this->panel.' Ne brisati. Ako želite da izbrišete podatke o studentima, molimo vas da označite grešku ispod i zatražite od administratora da prvo izbriše sve podatke.');
            return back()->withErrors($errors);
        }else{
            //remove images
            if (file_exists($this->folder_path.$row->student_image))
            ...
            $row->delete();
            $request->session()->flash($this->message_success, $this->panel.' Brisanje uspješno.');
        }
    ...
    public function active(request $request, $id)
    {
        ...
        $request->session()->flash($this->message_success, $row->reg_no.' '.$this->panel.' Aktivan uspjesno.');
        return redirect()->route($this->base_route);
    }
    ...
    public function inActive(request $request, $id)
    {
        ...
        $request->session()->flash($this->message_success, $row->reg_no.' '.$this->panel.' Neaktivan uspješno.');
        return redirect()->route($this->base_route);
    }
    ...
    public function findSemester(Request $request)
    {
        ...
        if ($request->has('faculty_id')) {
            ...
            if ($faculty) {
                ...
                $response['success'] = 'Semestar je dostupan za ovaj fakultet/odsjek.';
            } else {
                $response['error'] = 'Nema nijedan semestralni zadatak na ovom fakultetu/odsjeku.';
            }
        } else {
    ...
    public function transfering(Request $request)
    {
        if($request->faculty > 0 && $request->semester_select > 0){
            ...
            $request->session()->flash($this->message_success, 'Student transfer u : '.$faculty_title.' / '.$semester_title.' uspješno.');

        }else{
            $request->session()->flash($this->message_success, 'Molimo Vas da pravilno odaberete fakultet i semestar.');
        }
        return redirect()->route($this->base_route.'.transfer');
    }
    ...
    public function deleteAcademicInfo(Request $request, $id)
    {
        ...
        $request->session()->flash($this->message_success,'Akademske informacije uspješno izbrisane.');
    ...
    public function guardianInfo(Request $request)
    {
        ...
        if ($request->has('id')) {
            if ($guardianInfo = GuardianDetail::find($request->get('id'))) {
                ...
                $response['message'] = 'Operacija uspješna.';

            } else{
                $response['message'] = $request->get('subject_id').'Nevažeći zahtjev!!';
            }
        } else{
            $response['message'] = $request->get('id').'Nevažeći zahtjev!!';
        }
       ...
       // dd($request->all());
        ...
        foreach ($rows as $row) {
            ...
            $validator = Validator::make($row, [
                ...
                'mjesto_rodjenja'               => 'required | max:50',
                'opstina_rodjenja'              => 'required | max:30',
                'drzava_rodjenja'               => 'required | max:30',
                ...
                'broj_dosijea'                  => 'required | max:20',
                'status_studenta'               => 'max:25',
                'promjena_prezimena_imena'      => 'max:500',
            ]);
            ...
            $datum_ispisa               = Carbon::parse($row['datum_ispisa'])->format('Y-m-d');
            $datum_zavrsetka_studija    = Carbon::parse($row['datum_zavrsetka_studija'])->format('Y-m-d');
            //Student import
        ...
        $request->session()->flash($this->message_success,'Uvezeni studenti uspješno');
        return redirect()->route($this->base_route);
    }
...
/*                ->orwhere('students.broj_dosijea', 'like', '%'.$request->get('q').'%')  */
...
/*                ->orWhere('students.mjesto_rodjenja', 'like', '%'.$request->get('q').'%') */
                ->join('addressinfos as ai','ai.students_id','=','students.id')
                ->get();
            ...
            if($request->get('bulk_action') == 'export-excel'){
                ...
                if ($request->has('chkIds')) {
                    ...
                    if ($request->get('bulk_action') == 'active' || $request->get('bulk_action') == 'in-active') {
                        ...
                        $request->session()->flash($this->message_success, 'Prijava studenta Kreiraj/Resetuj uspješna.');
                    }elseif($request->get('bulk_action') == 'create-reset-library-member'){
                        $request->session()->flash($this->message_success, 'Član biblioteke uspješno kreiran.');
                    }else {
    ...
    public function createResetLogin($id)
    {
        ...
        if($student){
            ...
            if($existUser){
                ...
                $subject = 'Resetovanje prijave';
                $message = 'Poštovani ' . $name.', Vaši podaci za prijavu su ažurirani u našem sistemu. Molimo prijavite se sa  <br> email: '.$emailIds.' i lozinkom: '. $password ;
            }else{
                $user = User::create([
                    'role_id' => $rolesId,
                    'hook_id' => $student->id,
                ...
                $subject = 'Detalji prijave kreirani';
                $message = 'Poštovani ' . $name.', Vaši podaci za prijavu kreirani su u našem sistemu. Molimo prijavite se sa  <br> email: '.$emailIds.' i lozinkom: '. $password ;
            }
    ...
    public function generatePdf($id)
    {
        ...
        $data['student'] = Student::select('students.id', 'students.reg_no', 'students.reg_date', 'students.university_reg',
            ...
            'students.mother_tongue', 'students.email', 'students.extra_info', 'students.status', 'students.mjesto_rodjenja', 'drzava_rodjenja', 'students.broj_dosijea',
            'students.nacionalnost', 'students.status_studenta', 'students.promjena_prezimena_imena', 'students.dolazak_sa_druge_vsu',
            'students.nostrifikacija', 'students.datum_ispisa', 'students.promjene_studija', 'students.pohvale_nagrade', 'students.mjere',
            'students.datum_zavrsetka_studija',
