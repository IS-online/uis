...
class StaffController extends CollegeBaseController
{
    ...
    public function view($id)
    {
        ...
        if (!$data['staff']){
            request()->session()->flash($this->message_warning, "Nije valjan uposlenik");
            return redirect()->route($this->base_route);
        }
    ...
    public function delete(Request $request, $id)
    {
        ...
        if($userInfo) {
            $errCount = $errCount+1;
            $errors[] = "Pronađen korisnik, izbrišite.";
        }
        ...
        //Document & Notes
            ...
            if($document->count() > 0){
                $errCount = $errCount+1;
                $errors[] = "Dokumenti pronađeni, molimo izbrišite.";
            }
            ...
            if($notes->count() > 0){
            $errCount = $errCount+1;
            $errors[] = "Bilješke pronađene, molimo izbrišite.";
        }
        ...
        if($payrollMaster->count() > 0){
            ...
            if($paySalary->count() > 0){
                $errCount = $errCount+1;
                $errors[] = "Isplaćena plata pronađena, izbrišite.";
            }
            $errCount = $errCount+1;
            $errors[] = "Plaća Master pronađen, molimo izbrišite.";
        }
        ...
        if($transportUser->count() > 0){
            ...
            if($transportHistory->count() > 0){
                $errCount = $errCount+1;
                $errors[] = "Pronađena istorija transporta, molimo izbrišite.";
            }
            $errCount = $errCount+1;
            $errors[] = "Pronađen korisnik transporta, molimo izbrišite.";
        }
        ...
        if($hostelResident->count() > 0){
            ...
            if($residentHistory->count() > 0){
                $errCount = $errCount+1;
                $errors[] = "Pronađena istorija rezidenta hostela, molimo izbrišite.";
            }
            $errCount = $errCount+1;
            $errors[] = "Pronađen stanovnik hostela, molimo izbrišite.";
        }
        ...
        if($attendacne->count() > 0){
            $errCount = $errCount+1;
            $errors[] = "Pronađeno vannastavno prisustvo, molimo izbrišite.";
        }
        ...
        if($libraryMember->count() > 0){
            ...
            if($bookIssue->count() > 0){
                $errCount = $errCount+1;
                $errors[] = "Pronađena knjiga, molimo izbrišite.";
            }
            $errCount = $errCount+1;
            $errors[] = "Pronađen član biblioteke, izbrišite.";
        }

        if($errCount > 0){
            $request->session()->flash($this->message_warning, $this->panel.' Ne brisati. Ako želite da izbrišete podatke o studentima, provjerite err ispod i prvo izbrišite sve podatke.');
    ...
    public function active(request $request, $id)
    {
        ...
        $request->session()->flash($this->message_success, $row->reg_no.' '.$this->panel.' Aktivan uspješno.');
        return redirect()->route($this->base_route);
    }
    ...
    public function inActive(request $request, $id)
    {
        ...
        $request->session()->flash($this->message_success, $row->reg_no.' '.$this->panel.' Uspješno neaktivan.');
        return redirect()->route($this->base_route);
    }
    ...
    public function handleImportStaff(Request $request)
    {
        ...
        $request->session()->flash($this->message_success,'Uposlenici su uspješno uvezeni');
        return redirect()->route($this->base_route);
            ...
            if ($request->has('chkIds')) {
                if($request->get('bulk_action') == 'export-excel'){
                    ...
                    if ($request->get('bulk_action') == 'active' || $request->get('bulk_action') == 'in-active') {
                        ...
                        $request->session()->flash($this->message_success, 'Prijava studenta Kreiraj/Resetuj uspješno.');
    ...
    public function createResetLogin($id)
    {
        ...
        if($staff){
            ...
            if($existUser){
                ...
                $subject = 'Resetovanje detalja prijave';
                $message = 'Poštovani ' . $name.', Vaši podaci za prijavu ažurirani su u našem sistemu. Molimo prijavite se sa  <br> email: '.$emailIds.' i lozinka: '. $password ;
            }else{
                $user = User::create([
                    'role_id' => $rolesId,
                    'hook_id' => $staff->id,
                ...
                $message = 'Poštovani ' . $name.', Vaši podaci za prijavu kreirani su u našem sistemu. Molimo prijavite se sa  <br> email: '.$emailIds.' i lozinka: '. $password ;
            }
