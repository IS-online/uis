                    ...
                    {!! Form::open(['route' => $base_route.'.bulk-action', 'id' => 'bulk_action_form']) !!}
                        <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                                ...
                                                <div class="action-buttons">
                                                    <div class="btn-group">
                                                        <button data-toggle="dropdown" class="btn btn-primary btn-minier dropdown-toggle {{ $visitor->status == 'active'?"btn-success":"btn-warning" }}" >
                                                            {{ $visitor->status == 'active'?"Posjeta":"Završeno" }}
                                                            <span class="ace-icon fa fa-caret-down icon-on-right"></span>
                                                        </button>

                                                        <ul class="dropdown-menu">
                                                            <li>
                                                                <a href="{{ route($base_route.'.active', ['id' => $visitor->id]) }}"> Posjeta</a>
                                                            </li>

                                                            <li>
                                                                <a href="{{ route($base_route.'.in-active', ['id' => $visitor->id]) }}"> Završeno</a>
                                                            </li>
                                                        </ul>
                                                    </div>
                                ...
                                @else
                                    <tr>
                                        <td colspan="9">Podaci nisu pronađeni. Filtrirajte za prikaz. </td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
