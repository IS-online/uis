<div class="row">
    <div class="col-xs-12">
        <h4 class="header large lighter blue"><i class="fa fa-list" aria-hidden="true"></i>&nbsp;Naknade</h4>
        <div class="clearfix">
            ...
            <span class="hidden-print">
                <a class="btn-primary btn-sm" href="{{ route('account.fees.collection.view', ['id' => $data['student']->id]) }}">
                     <i class="fa fa-calculator" aria-hidden="true"></i> Glavna knjiga
                 </a>
            </span>
            ...
            <div class="row text-uppercase">
                <div class="col-sm-5 pull-right align-right">
                    {{--<strong>Saldo :</strong>{{$data['student']->balance}}/---}}
                    <label class="label label-info label-lg white">Total Balance : {{ number_format($data['student']->balance, 2) }}/-</label>
                </div>
                <div class="col-sm-7 pull-left">

                    <strong>Saldo slovima:</strong> {{ ViewHelper::convertNumberToWord($data['student']->balance) }}only.
                </div>
            </div>
                            ...
                            <tr>
                                ...
                                <td align="left" class="text text-left">
                                    ...
                                    @elseif($net_balance < 0 )
                                        <span class="label label-warning">Minus</span>
                                    @elseif($net_balance < $feemaster->fee_amount)
                                        <span class="label label-info">Parcijalno</span>
                                    @else
                                        <span class="label label-danger">Dužan</span>
                                    @endif
                                </td>
                                <td>
